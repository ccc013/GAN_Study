...
# -*- coding: utf-8 -*-
"""
@Time    : 2018/12/15 19:13
@Author  : cai

code mainly from https://github.com/bamos/dcgan-completion.tensorflow/blob/master/model.py

 implement a DCGAN model
"""
...
class DCGAN(object):
    ...
    def build_model(self):
        ...
        self.contextual_loss += tf.reduce_sum(
            tf.contrib.layers.flatten(
                tf.abs(
                    tf.multiply(self.lowres_mask, self.lowres_G) - tf.multiply(self.lowres_mask, self.lowres_images))),
            1)
